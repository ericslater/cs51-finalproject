...
struct 
   ...
   let calculate_massey data =
     ...
     let teams_list = team_list data in
     let indexed_list = assignment teams_list 0 in
     (* second entry point for passed in data *)
     let updated_data = update_index data indexed_list 0 in
     let massey_matrix = populate_massey updated_data in
     (* third entry point for passed in data *)
     let points = massey_point_spread data in
     let left_side = Matrix.multiply (Matrix.transpose massey_matrix) 
				     massey_matrix in
     let right_side = Matrix.multiply (Matrix.transpose massey_matrix) 
				      points in
   ...
   let minton_point_spread (stats: (string * int) list list)
     ...
     let rec mps_helper (teams: (string * int) list) :
       ...
       | cur_team :: league -> let (team, _) = cur_team in 
			       let ps1 = (create_spread stats team 0. 0.)
                               in
			       Matrix.append (Matrix.fill ps1)
   ...
   let calculate_minton data =
     ...
     (* third entry point for passed in data *)
     let points = minton_point_spread data in
   ...
   let calculate_colley data =
     (* first entry point for passed in data *)
     let teams_list = team_list data in
     let indexed_list = assignment teams_list 0 in
     (* second entry point for passed in data *)
     let updated_data = update_index data indexed_list 0 in
     let colley_matrix = populate_colley updated_data data in
     (* third entry point for passed in data *)
     let points = colley_point_spread data in
